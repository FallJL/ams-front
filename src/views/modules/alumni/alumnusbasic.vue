  ...
  <div class="mod-config">
    ...
    <div class="buttonBox">
      ...
      <el-alert title="温馨提示：" type="warning" :closable="false">
        <div>
          <p class="el-alert__description">
            导入时，请先点击“读取excel文件”，待下方表格读取数据完毕后，再点击“全部导入到数据库”
          </p>
          <p class="el-alert__description">
      ...
      <el-upload
        ...
        :on-change="handle"
      >
        <el-button type="success" slot="trigger" icon="el-icon-upload2"
          >读取excel文件</el-button
        >
        <el-button
          ...
          >下载excel导入模板</el-button
        >
        <el-button
          type="primary"
          @click="beforeSubmitDialogVisible = true"
          icon="el-icon-circle-plus-outline"
          >全部导入到数据库（无需勾选）</el-button
        >
        <el-button type="warning" @click="exports" icon="el-icon-download"
          >将勾选数据导出为excel</el-button
        >
      </el-upload>
      <el-dialog
        title="提示"
        :visible.sync="beforeSubmitDialogVisible"
        width="30%"
        :before-close="handleSubmitClose"
      >
        <span>若数据库中已有重复的学号，系统自动全部覆盖，还是手动选择？</span>
        <span slot="footer" class="dialog-footer">
          <el-button @click="() => beforeSubmit(false)">手动选择</el-button>
          <el-button type="primary" @click="() => beforeSubmit(true)"
            >自动覆盖</el-button
          >
        </span>
      </el-dialog>
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            type="text"
            size="small"
            @click="disableOrEnableHandle(scope.row.id, scope.row.aluStatus)"
          >
    ...
    <add-or-update
      ...
      ref="addOrUpdate"
      @refreshDataList="conditionQuery"
    ></add-or-update>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      beforeSubmitDialogVisible: false
    };
  },
  components: {
    AddOrUpdate
  ...
  methods: {
    ...
    beforeSubmit(isAllCovered) {
      this.beforeSubmitDialogVisible = false;
      this.submit(isAllCovered);
    },
    // 弹窗右上角的X，取消submit
    handleSubmitClose(done) {
      this.$confirm("确认取消导入？")
        .then(_ => {
          done();
        })
    ...
    submit(isAllCovered) {
      ...
      //   message: "已上传完毕！",
      //   type: "success",
      //   showClose: this.conditionQuery()
      // });
      // };
      let loading = Loading.service({
        text: "正在导入中，时间可能稍长，请耐心等待...",
        background: "rgba(0,0,0,.5)"
      });
      let n = 0;
      let send = async () => {
        ...
        this.$http({
          ...
          if (data.exist === 0) {
            ...
            this.$message.error("服务器出现问题，请咨询管理员");
            loading.close();
          }
        });
      };

      // 下面是for循环的代码，但无法实现同步发送请求
      // const send = () => {
      //   // 针对每一行遍历处理
      //   for (let i = 0; i < this.tempData.length; i++) {
      //     let body = this.tempData[i];
      //     this.$http({
      //       url: this.$http.adornUrl("/sys/feign/selectById"),
      //       method: "post",
      //       data: this.$http.adornData(body, false)
    ...
    disableOrEnableHandle(id, aluStatus) {
      ...
      // })
      this.$confirm(
        `确定对[id=${id}]进行[${aluStatus == 1 ? "禁用" : "启用"}]操作?`,
        "提示",
        {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
